...
| `MEND_APIURL`            | string  |    No    | Empty String | The Mend **API** host, e.g. `api-saas.mend.io`. This is not the same host as `MEND_URL`: API 1.4 lives on the SCA app host, while 2.0/3.0 live on the API host. Required only for features that use Mend API 3.0. |
