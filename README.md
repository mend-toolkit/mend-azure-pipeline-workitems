...
| `MEND_SSLVERIFY`         | string  |    No    | `true` | TLS certificate verification for both the Mend and Azure DevOps calls. Leave it unset: `requests` ships a CA bundle and both hosts serve publicly trusted certificates, so a hosted pipeline agent verifies them with nothing to install. Set it to a **CA bundle path** only for a self-hosted agent behind a TLS-inspecting proxy, where the certificate presented is the proxy's. `false` skips verification as versions before this did, and is the only setting that produces urllib3's `InsecureRequestWarning`. |
