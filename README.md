...
| `MEND_EPSS`              | boolean |    No    | `false` | When `true`, adds EPSS score and Exploit Code Maturity to each work item, read from the **same Mend API 3.0 finding** that produced the work item. No extra call, no extra variable, no extra permission and no separate login. See [Enrichment: Reachability, EPSS and Exploit Code Maturity](#enrichment-reachability-epss-and-exploit-code-maturity) below. |
...
Set `MEND_EPSS: true` to add **EPSS score** and **Exploit Code Maturity** to each work item, and/or `MEND_REACHABILITY: true` to add **Reachability**, both read from the Mend API 3.0 finding the work item was built from. Each defaults to `false` and is controlled independently — an org without reachability analysis enabled can turn on `MEND_EPSS` alone. Leaving both unset changes nothing about existing behavior.

Enabling either costs you nothing else. Both values arrive on the finding the integration already reads to build the work item — there is no second call, no additional variable to set, no additional Mend permission to grant, and no additional login for the integration to perform.
...
>**_NOTE_**: Existing work items do not gain these fields retroactively. A work item is only updated on the next run after you turn on `MEND_EPSS` / `MEND_REACHABILITY` keep showing the old, shorter table until then. Every run reads each project's full current state, so the next run after you enable them updates every work item.
