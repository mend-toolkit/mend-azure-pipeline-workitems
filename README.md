...
| `MEND_EMAIL`             | string  |    No    | Empty String | The email of the Mend user whose `MEND_USERKEY` this is. Required only for features that use Mend API 3.0. Must be set together with `MEND_APIURL` — setting one without the other aborts the run. |
| `MEND_APIURL`            | string  |    No    | Empty String | The Mend **API** host, e.g. `api-saas.mend.io`. This is not the same host as `MEND_URL`: API 1.4 lives on the SCA app host, while 2.0/3.0 live on the API host. Required only for features that use Mend API 3.0. Must be set together with `MEND_EMAIL` — setting one without the other aborts the run. |
